...
class PangleDramaPlayActivity : GenericBindActivity<ActivityDramaPlayPangleBinding>() {
    ...
    private val needShowPlaylist by lazy { intent?.getBooleanExtra(GlobalConstants.EXTRA_KEY_COMMON_BOOLEAN, false) ?: false }
        ...
        lifecycleScope.launch(Dispatchers.Main) {
            ...
            shortPlay?.let { item ->
                withContext(Dispatchers.IO) {
                    shortPlayHistory = intent?.getParcelableExtra(GlobalConstants.EXTRA_KEY_DRAMA_HISTORY) ?: database.historyDao().getItemById(item.id.toString())
                    if (null != shortPlayHistory) {
                        val historyIndex = shortPlayHistory?.currentIndex ?: 1
                        currentIndex = maxOf(startIndex, historyIndex)
                        currentProgress = if (startIndex > historyIndex) startProgress else shortPlayHistory?.currentProgress ?: 0
                    } else {
                        shortPlayHistory = DramaHistoryEntity(
                            Gson().toJson(item),
                            item.id.toString(),
                            startIndex,
                            startProgress,
                ...
                binding.imageCollect.imageTintList = ColorStateList.valueOf(if (isCollected) ContextCompat.getColor(activity, R.color.color_red) else Color.WHITE)
                    ...
                    lifecycleScope.launch(Dispatchers.IO) {
                        ...
                        withContext(Dispatchers.Main) {
                            binding.imageCollect.imageTintList =
                                ColorStateList.valueOf(if (isCollected) ContextCompat.getColor(activity, R.color.color_red) else Color.WHITE)
                        }
                    }
                }
                ...
                binding.btnResolution.setOnClickListener {
                    ...
                    MaterialAlertDialogBuilder(activity)
                        .setTitle(getString(R.string.resolution))
                        .setItems((resolutions ?: arrayOf()).map { it.toString(VideoRef.TYPE_VIDEO) }.toTypedArray()) { dialog, which ->
    ...
    private fun changeSeekbarSize(isLarge: Boolean) {
        binding.seekbar.thumb = ContextCompat.getDrawable(activity, if (isLarge) R.drawable.seekbar_thum_large else R.drawable.seekbar_thum)
        binding.seekbar.progressDrawable = ContextCompat.getDrawable(activity, if (isLarge) R.drawable.seekbar_bg_large else R.drawable.seekbar_bg)
    }
    ...
    private fun showDramaFragment(shortPlay: ShortPlay) {
        ...
        supportFragmentManager.beginTransaction().add(R.id.fragment_container, detailFragment!!).show(detailFragment!!).commit()
    }
