...
class HomeFragment : GenericFragment<FragmentHomeBinding>() {
    ...
    override fun bindView(inflater: LayoutInflater, container: ViewGroup?) = run { FragmentHomeBinding.inflate(inflater, container, false) }

    override fun initUI() {
        viewPagerAdapter = object : FragmentStateAdapter(childFragmentManager, lifecycle) {
            override fun createFragment(position: Int): Fragment = HomeDramaListFragment.newInstance(categories[position].id)
    ...
    private suspend fun requestCategoryList(): List<ShortPlayCategory>? = suspendCancellableCoroutine { continuation ->
        PSSDK.requestCategoryList(PSSDK.getContentLanguages()?.getOrNull(0) ?: "", object : PSSDK.CategoryListResultListener {
