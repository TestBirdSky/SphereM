...
import com.sphere.shortvideos.helper.mmkv.MMKVRepository.isBuyUser
...
class FbAndAdjustHelper {
    ...
    fun initFb(str: String) { //        val js = str.ifBlank { defC }
        //        runCatching {
        //            JSONObject(js).apply {
    ...
    fun initAdjust(context: Context) {
        val environment =
            AdjustConfig.ENVIRONMENT_SANDBOX //        if (BuildConfig.DEBUG) AdjustConfig.ENVIRONMENT_SANDBOX else AdjustConfig.ENVIRONMENT_PRODUCTION // todo modify adjust key
        ...
        config.setOnAttributionChangedListener { attribution ->
            logError("setOnAttributionChangedListener--->$attribution") // 判断用户类型：如果 network 包含 "Organic" 则为黑名单用户，否则为买量用户
            ...
            MMKVRepository.isBuyUser = isBlacklist.not()
            ...
            if (MMKVRepository.isBlacklistUser) {
                AppLifecycleManager.onDelayOnBackTime = 3_000
            } else {
                AppLifecycleManager.onDelayOnBackTime = 1_000
            }
    ...
    private fun parsePaidChannelByAdjustNetwork(network: String): String {
        ...
        return when {
            lower.contains("mintegral") || lower.contains("mtg") -> "mtg" // Facebook / Meta
            lower.contains("facebook", true) || lower.contains("meta") || lower.contains("instagram",
                true) || lower.contains("fb4a") -> "fb" // TikTok / ByteDance 常见写法
            lower.contains("tiktok", true) || lower.contains("bytedance",
                true) || lower.contains("pangle") || lower.contains("tik tok") -> "tt"
